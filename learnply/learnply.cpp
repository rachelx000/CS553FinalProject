...
// toggle for scalar field topology visualization
bool draw_stopo = false;
// toggle for critical point visualization
bool draw_critpoints = false;
// toggle for critical contour visualization
bool draw_critcontours = false;
...
void keyboard(unsigned char key, int x, int y) 
{
	...
	break;
	case 'p': // toggle critical point vis
	{
		draw_critpoints = !draw_critpoints;
		glutPostRedisplay();
	}
	break;
	case 'a': // toggle critical contour vis
	{
		draw_critcontours = !draw_critcontours;
		glutPostRedisplay();
	}
	break;
	...
	break;
	case 'n': // draw evenly spaced contours given
	{
		int n = 0;
...
void display(void)
{
	...
	if (draw_stopo)
	{
		...
		if (draw_critcontours)
			stopo->drawCrtitContours(use_height);
