...
void ScalarTopology::calcSpacedContours(int num)
{
    spaced_contours.clear();

    // divide the scalar range into N even intervals
    double step = (scalar_max - scalar_min) / (double)(num + 1);
    for (int i = 0; i < num; i++)
    {
...
void ScalarTopology::calcCriticals()
{
    ...
    // first loop through all the quads to find critical points occuring in 
    // the interior of a quad
    for (int i = 0; i < m_poly->nquads; i++)
    {
        ...
        // use billinear interpolation to get the scalar value at (x0, y0)
        ...
        // The Hessain matrix will help us classify the critical point
        double b = (f11 - f21 - f12 + f22) / ((x2 - x1) * (y2 - y1));
        // The Hessain matrix has zeros on the diagonal, so the eigenvalues
        // are -2b and 2b. We've already known that (f11 - f21 - f12 + f22)!= 0 
        // so b != 0
    ...
    // Then, loop through all the vertices to check for local mins and maxs
    for (int i = 0; i < m_poly->nverts; i++)
    {
        ...
        {
            double s = cp.loc.z;
            crit_contour.push_back(calcContour(s));
        }
    }
}
...
void ScalarTopology::drawCrtitContours(bool use_height)
{
    if (crit_contour.empty())
        return;
    ...
    for (int i = 0; i < criticals.size(); i++)
	{
		...
		PolyLine contour = crit_contour[i];
